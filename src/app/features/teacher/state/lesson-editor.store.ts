...
const toCreatePayload = (lesson: LessonDraft) => ({
  ...
  subcapitols: lesson.modules.length > 0
    ? lesson.modules.map((m, idx) => ({ title: m.title.trim() ? m.title : 'Untitled Module', orderIndex: idx + 1 }))
...
const mapFromResponse = (saved: Record<string, any>, fallbackModules: LessonModuleDraft[]): LessonDraft => ({
  ...
  modules: ((saved['subcapitols'] ?? saved['modules']) as any[])?.map((s: any, idx: number) => {
    ...
    return {
      ...
      content: textBlock?.content ?? fallback?.content ?? '',
      blockId: textBlock?.id ?? fallback?.blockId,
    };
  }) ?? fallbackModules,
});
  ...
  withMethods((
      ...
      if (err instanceof HttpErrorResponse && err.error) {
        ...
        if (typeof err.error === 'object' && err.error !== null) {
          const messages = Object.entries(err.error)
            .filter(([_, msg]) => typeof msg === 'string')
    ...
    const persist = async (lessonToSave: LessonDraft): Promise<LessonDraft> => {
      ...
      for (const mod of currentModules) {
        ...
        if (isNewSubcapitol) {
          const subRes: any = await lastValueFrom(http.post(`${apiBase}/lessons/${lessonId}/subcapitols`, { 
            title: safeTitle
          }));
          subcapitolId = subRes.id;

          const blockRes: any = await lastValueFrom(http.post(`${apiBase}/subcapitols/${subcapitolId}/blocks`, {
            blockType: 'TEXT', content: safeContent, mediaId: null, languageTag: 'ro', codeLanguage: null
          }));
          blockId = blockRes.id;
        } else {
          await lastValueFrom(http.put(`${apiBase}/subcapitols/${subcapitolId}`, { 
            title: safeTitle
          }));

          if (blockId) {
            await lastValueFrom(http.put(`${apiBase}/blocks/${blockId}`, {
              content: safeContent, codeLanguage: null, mediaId: null
            }));
          } else {
            try {
              const blockRes: any = await lastValueFrom(http.post(`${apiBase}/subcapitols/${subcapitolId}/blocks`, {
    ...
    return {
        ...
        http.get<unknown>(`${apiBase}/lessons/${id}`).subscribe({
          next: (saved) => {
            patchState(store, {
              ...
              loading: false, saveState: 'saved', lastSavedAt: new Date(),
            });
          },
          error: (err: unknown) => {
      ...
      addModule() {
        ...
        const newModule: LessonModuleDraft = { id, title: 'New Module', type: 'text', content: '', blockId: undefined };
      ...
      updateModule(id: string, patch: Partial<LessonModuleDraft>) {
        patchState(store, (s) => ({
          lesson: { ...s.lesson, modules: s.lesson.modules.map((m) => (m.id === id ? { ...m, ...patch } : m)) },
        }));
        markUnsaved();
      },
      ...
      async removeModule(id: string) {
        ...
        if (!id.startsWith('module-')) {
          try { await lastValueFrom(http.delete(`${apiBase}/subcapitols/${id}`)); } 
          catch (e) { console.error('Failed to delete subcapitol on backend', e); }
        }
        markUnsaved();
      },
      ...
      async save(onComplete?: (saved: LessonDraft) => void) {
        try {
          ...
          messageService?.add({ severity: 'success', summary: 'Saved', detail: 'Draft saved successfully.' });
      ...
      async publish(onComplete?: (saved: LessonDraft) => void) {
        try {
          ...
          messageService?.add({ severity: 'success', summary: 'Published', detail: 'Lesson is now visible to students!' });
      ...
      unpublish(onComplete?: (saved: LessonDraft) => void) {
        ...
        http.post<unknown>(`${apiBase}/lessons/${lesson.id}/unpublish`, {}).subscribe({
          next: (updated) => {
            ...
            patchState(store, { lesson: next, saveState: 'saved', lastSavedAt: new Date() });
            messageService?.add({ severity: 'info', summary: 'Unpublished', detail: 'Lesson returned to draft status.' });
            onComplete?.(next);
          },
          error: (err: unknown) => {
            const message = parseBackendError(err);
            patchState(store, { saveState: 'error', saveError: message });
            messageService?.add({ severity: 'error', summary: 'Unpublish Failed', detail: message });
          },
        });
      },
    };
