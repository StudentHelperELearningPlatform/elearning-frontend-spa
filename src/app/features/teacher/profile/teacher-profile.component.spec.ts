...
describe('TeacherProfileComponent', () => {
  ...
  let profileStore: InstanceType<typeof TeacherProfileStore>;
  ...
  beforeEach(async () => {
    ...
    profileStore = TestBed.inject(TeacherProfileStore);
  ...
  it('should create and load profile on init', () => {
    ...
    req.flush({
      ...
      classes: [{ id: 'cls-1', name: '9A' }],
    });

    fixture.detectChanges();

    expect(component).toBeTruthy();
    expect(profileStore.profile()?.name).toBe('Test Teacher');
  });
  ...
  it('should toggle editing mode', () => {
    ...
    expect(component.form.value.name).toBe('Test Teacher');
  ...
  it('should save profile successfully', () => {
    ...
    const updateReq = httpTestingController.expectOne(`${mockApiUrl}/teachers/me/profile`);
