...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, DragDropModule, CdkDragPreview, MediaPlayerComponent],
  styles: [
    `
  ...
  template: `
    <div class="p-4 border-2 border-black rounded-xl bg-gray-50">
      ...
      @if (errorMessage()) {
        <div
          class="mt-3 p-3 bg-red-100 text-red-900 border-2 border-red-500 rounded-lg flex items-center font-bold text-sm"
          role="alert"
        >
        ...
        <div class="mt-5 pt-4 border-t-2 border-black/10">
          ...
          >
            @for (media of mediaList(); track media.id) {
              <div
                cdkDrag
                class="relative w-40 bg-white border-2 border-black rounded-xl p-2 cursor-move group hover:-translate-y-1 transition-transform"
              >
                <ng-template cdkDragPreview>
                  <div
                    class="w-40 !h-fit !bg-white !border-2 !border-solid !border-black !rounded-xl !p-2 flex flex-col relative z-50 box-border"
                  >
                    ...
                    <p
                      class="text-xs font-bold text-gray-800 !mt-2 truncate text-center !px-1"
                      [title]="media.name"
                    >
                      {{ media.name }}
                    </p>
