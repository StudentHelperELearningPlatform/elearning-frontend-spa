  ...
  template: `
    <div class="p-2">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <h3 class="text-xl font-black tracking-tight text-black">
        ...
        <div class="flex flex-wrap gap-2">
          <button
            ...
            class="bg-white text-black px-4 py-2 rounded-xl font-bold border-2 border-black hover:bg-gray-100 disabled:opacity-50 transition-all flex items-center shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-y-[1px] hover:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] active:translate-y-[2px] active:shadow-none"
          >
          ...
          <button
            (click)="generateAI()"
            [disabled]="isAdding() || editingId() !== null || store.isGeneratingAI()"
            class="bg-[#0ABAB5] text-white px-4 py-2 rounded-xl font-bold border-2 border-black hover:bg-[#099994] disabled:opacity-50 transition-all flex items-center shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-y-[1px] hover:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] active:translate-y-[2px] active:shadow-none"
          >
      ...
      <div class="space-y-4">
        @for (q of store.questions(); track q.id; let i = $index) {
          @if (editingId() !== q.id) {
            <div
              class="bg-white border-2 border-black rounded-2xl p-5 flex flex-col md:flex-row gap-4 justify-between items-start shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"
            >
              <div class="flex-1">
                <div class="flex items-center gap-2 mb-3">
                  <span
                    class="text-xs font-black bg-gray-100 px-2 py-1 rounded-lg border-2 border-black shadow-[1px_1px_0px_0px_rgba(0,0,0,1)]"
                    >Q{{ i + 1 }}</span
                  >
                  @if (q.status === 'PENDING') {
                    <span
                      class="text-[10px] font-black uppercase tracking-wide bg-[#FFD700] text-black px-2 py-1 rounded-md border-2 border-black flex items-center shadow-[1px_1px_0px_0px_rgba(0,0,0,1)]"
                    >
                      <span class="material-icons text-[14px] mr-1">pending</span> Needs Approval
                    </span>
                  } @else if (q.status === 'APPROVED') {
                    <span
                      class="text-[10px] font-black uppercase tracking-wide bg-[#00FF7F] text-black px-2 py-1 rounded-md border-2 border-black flex items-center shadow-[1px_1px_0px_0px_rgba(0,0,0,1)]"
                    >
                ...
                <p class="font-black text-lg text-black mb-3">{{ q.questionText }}</p>
                ...
                <div class="space-y-1">
                  @for (opt of q.options; track opt.id) {
                    ...
                    >
                      <span
                        class="material-icons text-[18px] mr-2"
                        [class.text-[#0ABAB5]]="opt.correct || opt.optionText === q.correctAnswer"
              ...
              <div class="flex md:flex-col gap-2 shrink-0 mt-3 md:mt-0">
                @if (q.status === 'PENDING') {
                  <button
                    (click)="toggleApprove(q.id)"
                    class="px-3 py-2 border-2 border-black bg-[#00FF7F] text-black text-sm font-black uppercase rounded-xl hover:bg-[#00E673] flex items-center justify-center shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-y-[1px] hover:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] active:translate-y-[2px] active:shadow-none transition-all"
                  >
                ...
                <button
                  ...
                  class="px-3 py-2 border-2 border-black bg-white text-black text-sm font-black uppercase rounded-xl hover:bg-gray-100 flex items-center justify-center shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-y-[1px] hover:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] active:translate-y-[2px] active:shadow-none transition-all"
                >
                ...
                <button
                  ...
                  class="px-3 py-2 border-2 border-black bg-black text-white text-sm font-black uppercase rounded-xl hover:bg-gray-800 flex items-center justify-center shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-y-[1px] hover:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] active:translate-y-[2px] active:shadow-none transition-all"
                >
        ...
        @if (!store.isLoading() && store.questions().length === 0 && !isAdding() && !editingId()) {
          <div
            class="text-center p-10 border-2 border-dashed border-gray-400 rounded-2xl bg-gray-50"
          >
            <div
      ...
      @if (isAdding() || editingId() !== null) {
        <div
          class="mt-8 bg-gray-50 border-2 border-black rounded-2xl p-5 md:p-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"
        >
          <h4 class="font-black text-xl text-black mb-6 flex items-center">
          ...
          <label class="block mb-5">
            ...
            <textarea
              ...
              rows="2"
              class="w-full px-3 py-2 border-2 border-black rounded-xl font-bold text-base resize-none focus:ring-2 focus:ring-[#0ABAB5]/30 outline-none transition-all"
              ...
              @for (opt of newOptions(); track $index; let i = $index) {
                <div class="flex items-center gap-2">
                  ...
                  <input
                    type="text"
                    [(ngModel)]="opt.text"
                    class="flex-1 px-3 py-2 border-2 border-black rounded-xl font-bold text-sm focus:ring-2 focus:ring-[#0ABAB5]/30 outline-none transition-all"
                  ...
                  @if (newOptions().length > 2) {
                    <button
                      ...
                      class="w-11 h-11 shrink-0 flex items-center justify-center text-white bg-black border-2 border-black rounded-xl hover:bg-gray-800 transition-all"
                    >
          ...
          <div
            class="flex flex-col sm:flex-row justify-end gap-3 mt-6 pt-5 border-t-2 border-black"
          >
            <button
              (click)="cancelForm()"
              class="px-6 py-2 font-black text-black uppercase tracking-wide hover:bg-gray-200 border-2 border-transparent hover:border-black rounded-xl transition-all text-sm"
            >
              Cancel
            </button>
            <button
              (click)="submitQuestion()"
              class="px-6 py-2 bg-[#0ABAB5] text-white font-black uppercase tracking-wide border-2 border-black rounded-xl hover:bg-[#099994] transition-all shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-y-[1px] hover:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] active:translate-y-[2px] active:shadow-none text-sm"
            >
