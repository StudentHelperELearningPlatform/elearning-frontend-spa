import { Component, signal, inject } from '@angular/core';
import { CommonModule } from '@angular/common';
import { ReactiveFormsModule } from '@angular/forms';
import { AuthStore, User } from '@features/auth/store/auth.store';

// Import Shared Components
import { CardComponent } from '../../../shared/components/card/card.component';
import { AvatarComponent } from '../../../shared/components/avatar/avatar.component';
import { ButtonComponent } from '../../../shared/components/button/button.component';
import { FormFieldComponent } from '../../../shared/components/form-field/form-field.component';
import { BadgeComponent } from '../../../shared/components/badge/badge.component';

@Component({
  selector: 'app-profile',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
    CardComponent,
    AvatarComponent,
    ButtonComponent,
    FormFieldComponent,
    BadgeComponent,
  ],
  template: `
    <div
  class="max-w-6xl mx-auto p-4 md:p-8 font-sans bg-gray-50 text-black min-h-[calc(100vh-84px)]"
>
  <!-- HEADER -->
  <div class="flex items-center justify-between mb-8 border-b-4 border-black pb-4">
    <h1 class="text-4xl md:text-5xl font-black uppercase tracking-tighter">
      My Profile
    </h1>

    @if (!isEditing()) {
      <app-button variant="primary" icon="edit" (btnClick)="startEdit()">
        Edit Profile
      </app-button>
    } @else {
      <div class="flex gap-4">
        <app-button variant="secondary" (btnClick)="cancelEdit()">
          Cancel
        </app-button>

        <app-button variant="primary" icon="save" (btnClick)="saveProfile()">
          Save Changes
        </app-button>
      </div>
    }
  </div>

  <!-- GRID -->
  <div class="grid grid-cols-1 md:grid-cols-12 gap-8">

    <!-- LEFT SIDE -->
    <div class="md:col-span-4 space-y-8">
      <app-card>
        <div class="flex flex-col items-center text-center">

          <!-- AVATAR -->
          <div class="relative group mb-6">
            <app-avatar
              size="xl"
              [initials]="getInitials(user().name)"
            ></app-avatar>

            @if (isEditing()) {
              <button
                (click)="uploadAvatar()"
                class="absolute bottom-0 right-0 w-10 h-10 bg-[#0ABAB5] border-2 border-black rounded-full flex items-center justify-center text-white shadow hover:-translate-y-1 transition-all"
              >
                <span class="material-icons text-xl">photo_camera</span>
              </button>
            }
          </div>

          <!-- NAME -->
          <h2 class="text-2xl font-black uppercase tracking-tight mb-2">
            {{ user().name }}
          </h2>

          <!-- ROLE -->
          <app-badge variant="primary" class="mb-6">
            {{ authStore.role() || 'STUDENT' }}
          </app-badge>

          <!-- EMAIL + MEMBER -->
          <div class="w-full text-left space-y-4 pt-6 border-t-2 border-black/10">
            <div>
              <p class="text-xs font-black text-gray-500 uppercase tracking-widest">
                Email Address
              </p>
              <p class="font-bold text-lg truncate">
                {{ authStore.user()?.email }}
              </p>
            </div>

            <div>
              <p class="text-xs font-black text-gray-500 uppercase tracking-widest">
                Member Since
              </p>
              <p class="font-bold text-lg">August 2025</p>
            </div>
          </div>

        </div>
      </app-card>
    </div>

    <!-- RIGHT SIDE -->
    <div class="md:col-span-8 space-y-8">

      <!-- PERSONAL INFO -->
      <app-card header="Personal Information">

        @if (!isEditing()) {

          <!-- VIEW MODE -->
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-8">

            <div>
              <p class="text-xs font-black text-gray-500 uppercase tracking-widest mb-1">
                Full Name
              </p>
              <p class="text-xl font-bold">{{ user().name }}</p>
            </div>

            <div>
              <p class="text-xs font-black text-gray-500 uppercase tracking-widest mb-1">
                Phone Number
              </p>
              <p class="text-xl font-bold">{{ user().phone }}</p>
            </div>

            <div class="sm:col-span-2">
              <p class="text-xs font-black text-gray-500 uppercase tracking-widest mb-1">
                Bio
              </p>
              <p class="text-lg font-medium leading-relaxed">
                {{ user().bio }}
              </p>
            </div>

          </div>

        } @else {

          <!-- EDIT MODE -->
          <form class="space-y-6">

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">

              <app-form-field id="fullName" label="Full Name" icon="person">
                <input
                  type="text"
                  [value]="user().name"
                  (input)="user.update(u => ({ ...u, name: $any($event.target).value }))"
                  class="w-full border-4 border-black p-3 font-bold bg-white"
                />
              </app-form-field>

              <app-form-field id="phone" label="Phone Number" icon="phone">
                <input
                  type="text"
                  [value]="user().phone"
                  (input)="user.update(u => ({ ...u, phone: $any($event.target).value }))"
                  class="w-full border-4 border-black p-3 font-bold bg-white"
                />
              </app-form-field>

            </div>

            <app-form-field id="bio" label="Bio" icon="edit_note">
              <textarea
                rows="4"
                [value]="user().bio"
                (input)="user.update(u => ({ ...u, bio: $any($event.target).value }))"
                class="w-full border-4 border-black p-3 font-bold bg-white resize-none"
              ></textarea>
            </app-form-field>

          </form>

        }

      </app-card>

      <!-- PASSWORD -->
      <app-card header="Security Settings">
        <form class="space-y-6">

          <app-form-field id="currentPassword" label="Current Password" icon="lock">
            <input type="password" placeholder="Current password" />
          </app-form-field>

          <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">

            <app-form-field id="newPassword" label="New Password" icon="key">
              <input type="password" placeholder="New password" />
            </app-form-field>

            <app-form-field id="confirmPassword" label="Confirm Password" icon="key">
              <input type="password" placeholder="Confirm password" />
            </app-form-field>

          </div>

          <div class="pt-4 flex justify-end">
            <app-button variant="primary" icon="shield">
              Update Password
            </app-button>
          </div>

        </form>
      </app-card>

    </div>
  </div>
</div>
  `,
})
export class ProfileComponent {
  protected authStore = inject(AuthStore);

isEditing = signal(false);

// mock data (temporar)
user = signal({
  name: this.authStore.user()?.name || 'John Doe',
  phone: '+40 712 345 678',
  bio: 'Student bio...'
});

// backup pentru cancel
originalUser: any = null;

// ENTER EDIT
startEdit() {
  this.originalUser = JSON.parse(JSON.stringify(this.user()));
  this.isEditing.set(true);
}

uploadAvatar() {
  console.log('Upload avatar clicked');

  // pentru moment doar simulăm
  alert('Avatar upload (UI only)');
}

// SAVE
saveProfile() {
  console.log('PUT /api/users/me', this.user());

  this.isEditing.set(false);
  this.originalUser = null;

  alert('Profile updated successfully');
}

// CANCEL
cancelEdit() {
  const hasChanges =
    JSON.stringify(this.user()) !== JSON.stringify(this.originalUser);

  if (hasChanges) {
    const confirmDiscard = confirm(
      'You have unsaved changes. Discard?'
    );

    if (!confirmDiscard) return;
  }

  this.user.set(this.originalUser);
  this.isEditing.set(false);
}

  // Helper to extract initials for the avatar
  getInitials(name?: string): string {
    if (!name) return 'UN';
    const parts = name.trim().split(' ');
    if (parts.length >= 2) {
      return (parts[0][0] + parts[1][0]).toUpperCase();
    }
    return name.substring(0, 2).toUpperCase();
  }
}
